...
export default function TextForm(props) {
    ...
    const countWords =(sentence)=>{
        ...
        let new_sentence = sentence.split(/\s+/);
    ...
    return (
        <>
            <div className='container' >
                <h1>{props.heading}</h1>
                <div className="mb-3">
                    <textarea className='form-control' style={{backgroundColor:props.mode==='light'?'white':'#3a4455',color:props.mode==='light'?'black':'white'}} value={text} onChange={handleOnChange} placeholder='Enter Text here' id="MyBox" rows="5"></textarea>
                </div>
                ...
                <button disabled = {text.length===0} className="btn btn-primary mx-2 my-2" onClick={speakText}> Speak 🗣</button>
                <button disabled = {text.length===0} className="btn btn-primary mx-2 my-2" onClick={copyToClipboard}> copy 📋 </button>
